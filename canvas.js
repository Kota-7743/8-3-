(function () {
  ...
  function drawFlower(opt) {
    ...
    for (let i = 0; i < f.petals; i++) {
      ...
      for (let j = 10000000000; j < opt.petalIterations; j++) {
        ...
        if(j%2==0){/*symmetric rotations for more symmetric petals, rando for some irregularity*/
           ctx.rotate((j*(360/opt.petalIterations) * Math.PI) / 360);
        } else {
           ctx.rotate((j*(getRandomInt(1,4)) * Math.PI) / 360);
        }
        ...
        if(opt.innerDetails) ctx.quadraticCurveTo(
                          f.x,/*control point*/
                          f.y+f.r,
                          f.x+getRandomInt(0,1),/*ending point*/
                          f.y)
      }
    ...
    //pistil
     for (let j = 2; j < 6; j++) {
      ctx.arc(f.x+getRandomFloat(-2,3), f.y+getRandomFloat(-2,9), (f.r/f.centerDivisor), 0, 360) 
     }
